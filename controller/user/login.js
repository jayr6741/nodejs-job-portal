    ...
    try {
        ...
        if (!getData) {
            const obj = {
                res,
                status: 401,
                messages: "pleash register your email id",
            }
        ...
        if (getData.role !== "user") {
            const obj = {
                res,
                status: 401,
                messages: "Permission decline , you are not Admin"
            }
            return Response.Error(obj);
        }
        ...
        if (password == decrypt) {
            const encrypt = CryptoJS.AES.encrypt(getData.id.toString(), process.env.USER_KEY).toString();
